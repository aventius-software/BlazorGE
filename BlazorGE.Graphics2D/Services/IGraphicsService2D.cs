...
namespace BlazorGE.Graphics2D.Services
{
    public interface IGraphicsService2D : IGraphicsService
    {
        ...
        public ValueTask DrawFilledPolygonAsync(string fillColour, string strokeColour, int[,] coordinates);
        public ValueTask DrawFilledRectangleAsync(string colour, int x, int y, int width, int height);
        public ValueTask DrawImageAsync(string id, int x, int y, int width, int height, int sourceX, int sourceY, int sourceWidth, int sourceHeight);
